...
// ✅ Consistent token key — matches Login.jsx and Register.jsx
...
function SignOutDialog({ open, onConfirm, onCancel }) {
  return (
    <Dialog
      open={open}
      onClose={onCancel}
    ...
    >
      ...
      <DialogContent>
        <Typography variant="body2" color="text.secondary">
          You'll need to sign back in to access your orders and saved items.
        </Typography>
      </DialogContent>
...
function NavigationBar({ cartItemCount }) {
  ...
  const [signOutOpen, setSignOutOpen] = React.useState(false); // ✅ sign out dialog state
  ...
  // ✅ Fixed: now reads 'swastikToken' consistently
  React.useEffect(() => {
    const checkToken = () => {
        ...
        {isMobile ? (
          <>
            ...
            <Menu id="mobile-menu" anchorEl={anchorEl} open={open} onClose={handleClose}>
              <MenuItem onClick={() => { handleClose(); handleSearchModalOpen(); }} sx={{ gap: 0.5 }}>
                Search
              </MenuItem>
              <Divider sx={{ my: 0.5 }} />
              {navLinks.map(link => (
                <MenuItem key={link.to} onClick={() => { handleClose(); navigate(link.to); }}>
                  {link.label}
                </MenuItem>
              ))}
              <MenuItem onClick={() => { handleClose(); navigate('/cart'); }}>
                <Badge
            ...
            <Typography variant="h6" component="div" sx={{ flexGrow: 1, minWidth: 0 }}>
              <Link to="/" className="logo-link">SWASTIK ELECTRONICS</Link>
            </Typography>
          </>
        ) : (
          <>
            ...
            <form className="search-bar" ref={searchBarRef} onSubmit={e => e.preventDefault()}>
              ...
              />
              {loading && <CircularProgress size={18} sx={{ color: 'white', ml: 1 }} />}
            </form>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, ml: 'auto' }}>
              ...
              {isLoggedIn ? (
                // ✅ Now opens confirmation dialog instead of logging out immediately
                <Tooltip title="Sign out" arrow>
                  <IconButton
                    color="inherit"
                    size="medium"
                    onClick={handleLogoutClick}
                    sx={{ color: '#ff6b6b', '&:hover': { color: '#ff4444' } }}
                  >
                ...
                <>
                  <Tooltip title="Sign in" arrow>
                    <IconButton component={Link} to="/login" color="inherit" size="medium">
                      <LoginIcon fontSize="medium" />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="Create account" arrow>
                    <IconButton component={Link} to="/register" color="inherit" size="medium">
              ...
              <Tooltip title="View cart" arrow>
                <IconButton color="inherit" component={Link} to="/cart" size="medium">
